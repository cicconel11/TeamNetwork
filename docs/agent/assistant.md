...
The assistant now supports confirmation-gated write paths for announcement create/update/delete, jobs, direct and group chat messages, top-level discussion threads, discussion replies, and calendar events. Announcement update/delete is implemented as pending-action tooling (`prepare_update_announcement`, `prepare_delete_announcement`) and confirmation-time handlers, but the routing surface still needs broader end-to-end coverage to prove natural-language edit/delete prompts always reach those tools instead of navigation fallback. Broader write parity for role changes, forms, settings, and other admin mutations is still not implemented.
...
Simple roster, event, donation-list, chat-group, and stats questions now take fast single-tool paths, and thread listing / message history queries are covered by dedicated composite indexes (`idx_ai_threads_org_listing`, `idx_ai_messages_thread_status`). Remaining latency is concentrated in a few places:

- `get_org_stats` full snapshots still fan out across member, alumni, parent, event, and donation sources. Scope-derived calls now avoid unrelated slices when the prompt is narrow, but generic snapshot prompts still pay for the full aggregate. Next optimization: add a compact stats materialized view/RPC for the dashboard-style snapshot or split full snapshots into cached slices with short TTLs.
- `search_org_content` now calls the shared RPC and then direct announcement/event fallback queries. That improves recall, but it means every content-search turn can do three DB reads. Next optimization: fold fallback title/body matching into the RPC or run fallback only when the RPC returns too few rows for the requested entity type.
- Deterministic `tool_first` responses still sometimes perform small post-tool lookups (`resolveOrgSlug` for chat groups, donor privacy for donations). Next optimization: include those formatting flags in the tool payload so rendering does not need extra round trips.
- Pass-1 bypass is limited to zero-arg/derivable read tools. Next optimization: extend the bypass allowlist to deterministic search/navigation cases once query extraction is local and covered by routing tests.
- Prompt context loads org info even in lightweight `tool_first` turns. Next optimization: make `tool_first` context explicitly minimal and reuse already-loaded org slug/name from auth/org context where possible.
