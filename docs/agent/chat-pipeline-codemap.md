...
- The shipped write paths are `prepare_announcement`, `prepare_update_announcement`, `prepare_delete_announcement`, `prepare_job_posting`, `prepare_chat_message`, `prepare_group_message`, `prepare_discussion_reply`, `prepare_discussion_thread`, `prepare_event`, and `prepare_member_role_change`, all backed by the same pending-action confirm/cancel routes.
