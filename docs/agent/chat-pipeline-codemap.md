...
The chat pipeline handles the full lifecycle of an AI chat request: rate limiting, active-admin auth, input validation, message-safety assessment, thread management, semantic cache check, prompt construction, conditional tool attachment, LLM streaming via SSE, message persistence, cache write-back, audit logging, and deterministic grounding enforcement for tool-backed summaries. A small internal `TurnExecutionPolicy` layer now centralizes cache, retrieval, context, and tool decisions from existing routing signals instead of spreading them across handler branches. Tool execution is now defense-in-depth hardened in the executor itself, and each turn stage is bounded so pass 1, each tool call, and pass 2 cannot hang indefinitely. The shipped read-tool set includes members, events, announcements, discussions, job postings, org stats, content search, connection suggestions, and `find_navigation_targets` for page/deep-link lookup. Navigation/action requests short-circuit to `find_navigation_targets` instead of routing through the broader read-tool set, content-search prompts can short-circuit to `search_org_content`, while announcement-creation prompts can attach `prepare_announcement`, reply prompts can attach `prepare_discussion_reply`, job create/update/delete prompts can attach `prepare_job_posting`, `prepare_update_job_posting`, or `prepare_delete_job_posting`, event create/update/delete prompts can attach `prepare_event`, `prepare_update_event`, or `prepare_delete_event`, member role/status prompts can attach `prepare_member_role_change`, and discussion-creation prompts can attach `prepare_discussion_thread` to validate a draft and emit a structured `pending_action` confirmation event. Confirmation-time side effects now run server-side with the confirm handler's service client rather than bouncing through internal authenticated `fetch()` calls, so AI-created announcements and events can still deliver notifications and calendar sync when no browser cookies are present. Announcement and reply routing now also recognize broader natural-language phrasing like `write` / `compose` / `response`, and scoped route-context fallback can still attach `prepare_announcement` on announcement pages, `prepare_member_role_change` on member detail pages, `prepare_discussion_reply` on discussion-thread pages, or event/job update/delete tools on matching detail pages when the prompt is clearly a draft follow-up but misses the narrow first-pass regex. Schedule-import turns can now also attach the legacy-named `extract_schedule_pdf` tool for either uploaded PDFs or uploaded schedule images, and website imports still use `scrape_schedule_website`. Uploaded schedule files are treated as transient private attachments: the upload route validates PDF/image signatures before storing them, self-heals the private bucket if it is missing or still on the old PDF-only allowlist, exposes a `DELETE` cleanup endpoint for abandoned pending uploads, and the extractor deletes the storage object after it has been read into memory. Registry modules for `scrape_schedule_website` and `extract_schedule_pdf` lazy-load cheerio, PDF parsing, and the schedule extraction stack so plain chat turns do not evaluate those dependencies unless a schedule tool is actually invoked. Prompt construction still treats the client-reported current page path as untrusted input, but the handler now resolves a trusted route-entity snapshot from that path with the caller's RLS-scoped Supabase client and the current `organization_id`. That normalized entity context is memoized once per request, injected into the untrusted prompt context as a bounded `Route Entity` section, and reused for trusted tool-argument seeding on phrases like `message this person`, `change this person's role`, `reply to this thread`, `update this event`, or `delete this job`. If the route is malformed, cross-org, soft-deleted, archived/inactive, or not visible through RLS, the entity context is omitted and the turn falls back to prior behavior. `find_navigation_targets` now also returns deterministic `manualSteps` and `assistantCanHelpWith` guidance for each accessible result, so navigation answers can tell the user both where to go and what the assistant can still help do from chat. When that deterministic response has exactly one accessible internal target, the server emits a `navigation` SSE event and the client performs a safe `router.push()` while keeping the markdown link in the transcript. The route now also has a materially faster deterministic path for single-tool `tool_first` reads such as `list_members`, `list_events`, `get_org_stats`, `get_donation_analytics`, `list_donations`, `list_chat_groups`, `search_org_content`, `suggest_connections`, and `find_navigation_targets`; these turns reuse a minimal prompt context, can skip pass 2 entirely, and can bypass the pass-1 model round-trip when args are locally derivable. A pure fast-path classifier now tags the existing routing outcome with an additive `request.fast_path_label` for audit analysis without expanding bypass eligibility. Audit rows persist a `stage_timings` JSON payload with per-stage duration/status plus the final retrieval decision/reason for each logged turn, and dev admins can inspect aggregate-only latency buckets through `/api/admin/ai/latency-stats`.
...
| `src/lib/ai/route-entity-loaders.ts` | RLS-scoped org/entity loaders for route-derived prompt context | `loadRouteEntityContext` |
...
| `src/lib/ai/sse.ts` | SSE encoding, stream factory, event types including `navigation` events for safe client-side route changes | `CacheStatus` type, `SSEEvent` type, `encodeSSE`, `createSSEStream`, `SSE_HEADERS` |
...
| `src/lib/ai/chat-telemetry.ts` | Shared retrieval/stage timing contracts for AI audit payloads | `AiAuditStageTimings`, `AiAuditStageName`, `FastPathLabel` |
| `src/lib/ai/latency-stats.ts` | Pure aggregate latency telemetry builder over audit-like rows | `buildLatencyStats` |
...
| `src/app/api/ai/[orgId]/chat/handler/fast-path-classifier.ts` | Pure classifier for fast-path labels, compatible pass-1 path metadata, and tool-first context eligibility | `classifyFastPath` |
...
| `src/app/api/admin/ai/latency-stats/route.ts` | Dev-admin aggregate latency telemetry endpoint over capped `ai_audit_log` scans | `GET` |
...
| `supabase/migrations/20261027000000_ai_org_stats_snapshot.sql` | Adds compact `get_org_stats_snapshot(p_org_id uuid)` RPC for generic org-stat snapshots |
| `supabase/migrations/20261102000000_ai_audit_log_created_at_index.sql` | Adds `ai_audit_log(created_at DESC)` for latency endpoint `WHERE created_at >= since ORDER BY created_at DESC LIMIT 50000` scans |
...
Client POST /api/ai/{orgId}/chat
  ...
  │
  ├─ 8.6 Resolve current route entity once per request
  │       ├─ parse supported entity routes from client `currentPath`
  │       ├─ load compact entity context through caller-scoped RLS + organization filter
  │       ├─ drop malformed, cross-org, deleted, archived/inactive, or inaccessible rows
  │       └─ memoize the normalized result for prompt context and tool seeding
  │
  ├─ 8.7 CACHE CHECK (if policy allows lookup_exact)
  ...
  │       │   ├─ Route entity context is formatted only from the preloaded normalized route snapshot
  ...
  │       ├─ `classifyFastPath` centralizes the suppressor/enabler labels used for audit telemetry; it is pure and must not expand runtime eligibility by itself
  ...
  │       ├─ Explicit content-search asks can expose only `search_org_content`
  ...
  │       ├─ `find_navigation_targets` returns accessible org-scoped deep links for open/create/manage page requests plus deterministic next-step guidance
  ...
  │       ├─ Single-tool `list_members`, `list_events`, `get_org_stats`, `get_donation_analytics`, `list_donations`, `list_chat_groups`, `search_org_content`, `suggest_connections`, `list_announcements`, and `find_navigation_targets` results are rendered deterministically in-route
  ...
  │       ├─ `request.fast_path_label` is optional/additive at `schema_version: 1`; old rows without it are valid and latency stats bucket them as `unclassified`
...
- Differentiated rendering (`formatters/mentorship-suggestions.ts`): scores render as "Match score: N/100 (Label)"; reasons shared by every suggestion collapse into one compact "Common match signals across all N" line so per-person bullets only show differentiators; every mentor card gets a signal-driven "Best for" positioning line (career-stage gap relative to the list, then that person's unique signals, then role/company vantage — fully value-driven, org-agnostic); the top match gets a "Why X ranks #1" line (career-stage-fit copy when the ranking turns on a closer graduation gap); the mentor direction appends up to 3 conversation-starter questions for the top match (templates keyed by that match's reason codes); a thin-data caveat renders when every suggestion has ≤2 signals or any is `fallback_general`. Full reason text flows through `formatMatchExplanation` so claimed grounding codes stay backed; compact/Best-for/question copy deliberately avoids `REASON_CODE_LABEL_PATTERNS` phrases so it claims no codes.
...
- Member role/status changes preflight no-op, final-admin, and alumni/parent subscription gates before creating a pending action; confirmation writes through `src/lib/members/role-change.ts` and inserts `org_member_role_audit.source = 'ai_pending_action'` with the pending action id.
- Broader write tools like forms, settings changes, exports, billing mutations, and other admin operations are still deferred.
...
- Confirm responses add normalized metadata (`actionType`, `resultEntityType`, `resultEntityId`, `affectedPaths`, `affectedEvents`) from the confirm handler. The helper is intentionally local to that handler for now because only confirm branches produce this shape.
