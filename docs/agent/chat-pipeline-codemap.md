...
- Differentiated rendering (`formatters/mentorship-suggestions.ts`): reasons shared by every suggestion collapse into one "All N share: …" line so per-person bullets only show differentiators; the top match gets a "What sets X apart from Y" line against the runner-up; the mentor direction appends up to 3 conversation-starter questions for the top match (templates keyed by that match's reason codes, worded to never collide with `REASON_CODE_LABEL_PATTERNS`); a thin-data caveat renders when every suggestion has ≤2 signals or any is `fallback_general`. All reason text still flows through `formatMatchExplanation`, so grounding codes stay backed.
