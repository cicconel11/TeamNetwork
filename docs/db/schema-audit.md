...
**Scope**: All migrations through `20260425100000_push_notifications.sql` (44 migration files)
...
| Table | Purpose | Key Columns |
|-------|---------|-------------|
| `members` | Active member profiles | `id`, `organization_id`, `user_id`, `email`, `name`, `avatar_url`, `linkedin_url`, `deleted_at` |
| `alumni` | Alumni profiles with extended fields | `id`, `organization_id`, `user_id`, `email`, `name`, `avatar_url`, `linkedin_url`, `phone_number`, `industry`, `current_company`, `current_city`, `position_title`, `graduation_year`, `deleted_at` |
...
| `alumni` | Org members | Admin + `can_add_alumni()` | Admin or self (profile owner) | Admin only |
...
All SECURITY DEFINER functions use `SET search_path = ''` (or `= public`) to prevent search-path injection.
...
- All SECURITY DEFINER functions hardened with `SET search_path = ''` (`20260107120000`)
