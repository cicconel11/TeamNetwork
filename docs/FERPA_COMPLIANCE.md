...
- Subject identifier + identifier type (email / student ID / alumni ID)
- Request type (inspect, correct, delete, export)
- Request source (parent direct / school relay / student self-service / admin entered)
- Date received → date acknowledged (≤10 days) → date resolved (≤45 days)
- Acknowledgement method + resolution method (email / portal / school relay, etc.)
...
* [x] **Parents' Bill of Rights:** `/privacy` now links to parent-facing FERPA / NY transparency pages (`/privacy/parent-notification`, `/privacy/parents-bill-of-rights`) for district review.
...
- [x] Rights Request Workflow Tested — `tests/compliance/dsr-intake.test.ts`
...
| `compliance/dsr-intake.test.ts` | DSR intake schema, SLA helper, self-service export/delete logging, privacy-page FERPA routing copy |
