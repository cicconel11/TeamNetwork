...
export function EmbedManager({ orgId, embeds: initialEmbeds }: EmbedManagerProps) {
  const router = useRouter();
  const [embeds, setEmbeds] = useState<PhilanthropyEmbed[]>(initialEmbeds);
  ...
  const [formData, setFormData] = useState({
    title: "",
    url: "",
    embed_type: "link" as "link" | "iframe",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (data) {
        setEmbeds((prev) => [...prev, data as PhilanthropyEmbed]);
      }
    }
  ...
  const handleEdit = (embed: PhilanthropyEmbed) => {
    setFormData({
      title: embed.title,
      url: embed.url,
  ...
  return (
    <Card className="p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold text-foreground">Fundraising Embeds</h3>
          <p className="text-sm text-muted-foreground">
            Add external fundraising pages or embeddable content
          </p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="space-y-4 mb-6 p-4 rounded-xl bg-muted/50">
          ...
          <Select
            ...
            onChange={(e) =>
              setFormData({ ...formData, embed_type: e.target.value as "link" | "iframe" })
            }
            options={[
