...
import { canRenderAsIframe } from "@/lib/security/embed-url-policy";
    ...
    <div className="space-y-4 mb-6">
      {embeds.map((embed) => (
        <Card key={embed.id} className="overflow-hidden">
          {(() => {
            const iframePolicy = canRenderAsIframe(embed.url);
            const renderAsIframe = embed.embed_type === "iframe" && iframePolicy.ok;
            const blockedIframe = embed.embed_type === "iframe" && !iframePolicy.ok;
                      ...
                      <div>
                        ...
                        {blockedIframe && (
                          <p className="text-xs text-amber-700 mt-1">
                            Opened as link. Embedding is blocked by provider policy.
                          </p>
                        )}
                      </div>
                    </div>
