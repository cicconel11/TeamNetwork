...
export default async function RecordsPage({ params, searchParams }: RecordsPageProps) {
  ...
  const [tNav, locale, tRecords, tCommon] = await Promise.all([
      ...
      {records && records.length > 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-muted/50">
                        <th className="text-left font-medium text-muted-foreground px-4 py-3">{tRecords("record")}</th>
                        <th className="text-left font-medium text-muted-foreground px-4 py-3">{tRecords("holder")}</th>
                        <th className="text-left font-medium text-muted-foreground px-4 py-3">{tRecords("value")}</th>
                        <th className="text-left font-medium text-muted-foreground px-4 py-3 w-20">{tRecords("year")}</th>
                        {isAdmin && (
                          <th className="text-right font-medium text-muted-foreground px-4 py-3 w-28">{tCommon("actions")}</th>
                        )}
                      </tr>
                    </thead>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <SoftDeleteButton
                                  ...
                                  confirmMessage={tRecords("confirmDelete")}
                                />
                              </div>
                            </td>
                          )}
        ...
        <Card>
          <EmptyState
            ...
            title={tRecords("noRecords", { label: pageLabel.toLowerCase() })}
            description={tRecords("addRecords", { label: pageLabel.toLowerCase() })}
            action={
              isAdmin && (
                <Link href={`/${orgSlug}/records/new`}>
