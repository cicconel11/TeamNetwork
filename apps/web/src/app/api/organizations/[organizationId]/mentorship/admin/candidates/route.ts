...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  if (rpcError && rpcError.code !== "23505") {
    return NextResponse.json({ error: rpcError.message ?? "Failed to propose pair" }, { status: 500 });
  }
  ...
  if (!pairId) {
    return NextResponse.json({ error: "Pairing already exists" }, { status: 409 });
  }
  ...
  if (acceptError) {
    ...
    return NextResponse.json(
      { pair_id: pairId, status: "proposed", warning: acceptError.message },
      { status: 200 }
    );
  }
  ...
  const acceptedRow = (Array.isArray(acceptData) ? acceptData[0] : acceptData) as
    ...
    | null;
  return NextResponse.json(
    { pair_id: pairId, status: acceptedRow?.status ?? "accepted" },
    { status: 200 }
  );
}
