/**
 * Consolidated Schedule Domain Security Tests
 *
 * Tests for domain allowlist and verification including:
 * - Domain status checking (active, pending, denied, blocked)
 * - Domain enrollment and verification
...
import { checkHostStatus } from "../src/lib/schedule-security/allowlist.ts";

// Types for domain testing
type DomainRow = {
  id: string;
  hostname: string;
  vendor_id: string;
...
// Minimal Supabase stub for schedule_allowed_domains (checkHostStatus)
function createAllowlistStub(
  domains: Array<{
    id: string;
    hostname: string;
    vendor_id: string;
...
// Full Supabase stub for verifyAndEnroll testing
...
describe("Schedule Domain Security", () => {
  describe("Domain Status Checking", () => {
    it("should return verifiedByOrgId for pending domains", async () => {
      ...
      const result = await checkHostStatus("athletics.example.edu", undefined, stub as never);
    ...
    it("should return undefined verifiedByOrgId for active domains", async () => {
      const stub = createAllowlistStub([
        {
          id: "domain-2",
      ...
      const result = await checkHostStatus("sports.example.edu", undefined, stub as never);

      assert.strictEqual(result.status, "active");
      assert.strictEqual(result.verifiedByOrgId, undefined);
    });

    it("should return denied for unknown hosts", async () => {
      const stub = createAllowlistStub([]);

      const result = await checkHostStatus("unknown.example.com", undefined, stub as never);

      assert.strictEqual(result.status, "denied");
      assert.strictEqual(result.source, "none");
    });

    it("should allow org-b to see verifiedByOrgId when domain is pending from org-a", async () => {
      ...
      const result = await checkHostStatus("calendar.school.edu", undefined, stub as never);
  ...
  describe("Domain Enrollment and Updates", () => {
    it("should not downgrade active domains to pending", async () => {
    ...
    it("should allow upgrading pending domains to active", async () => {
    ...
    it("should allow refreshing already-active domains", async () => {
    ...
    it("should never update blocked domains regardless of nextStatus", async () => {
      const domains: DomainRow[] = [
        {
          id: "domain-4",
  ...
  describe("Concurrent Insert Race Handling", () => {
    it("should handle unique constraint violation on concurrent inserts", async () => {
