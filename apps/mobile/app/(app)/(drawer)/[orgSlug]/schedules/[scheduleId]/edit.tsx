  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        setSchedule(data as AcademicSchedule);
        // Populate form
        setTitle(data.title);
        setOccurrenceType(data.occurrence_type as OccurrenceType);
        setStartTime(parseTimeToDate(data.start_time));
        setEndTime(parseTimeToDate(data.end_time));
        setStartDate(parseDateStr(data.start_date));
        setEndDate(data.end_date ? parseDateStr(data.end_date) : null);
  ...
  if (loadingSchedule) {
    return (
      <View style={[styles.container, styles.centered]}>
        <ActivityIndicator size="large" color={SCHEDULE_COLORS.primaryCTA} />
      </View>
    );
  }
