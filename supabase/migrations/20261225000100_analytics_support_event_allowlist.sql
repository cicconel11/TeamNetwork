-- Apple App Review reframe (part 2): allow-list the new support_* event props.
-- Mirrors the donation_* arms exactly (same prop shapes) and keeps the donation_*
-- arms for backward compatibility with any in-flight clients. Separate migration
-- from the ADD VALUE statements so the new enum values are committed before this
...
BEGIN
  ...
  allowed_keys := CASE p_event_name
    ...
    WHEN 'support_flow_start' THEN ARRAY['campaign_id']
    WHEN 'support_checkout_start' THEN ARRAY['campaign_id','amount_bucket']
    WHEN 'support_checkout_result' THEN ARRAY['campaign_id','result','error_code']
