-- Add advisory lock to serialize concurrent invite creation per enterprise
-- Issue 2: Prevents race condition where parallel bulk invites bypass admin cap
--   and alumni quota checks by reading the same count simultaneously.
--
-- The pg_advisory_xact_lock is released automatically at transaction end,
-- ensuring every create_enterprise_invite call for the same enterprise is
-- serialized through the admin cap and alumni quota checks.
