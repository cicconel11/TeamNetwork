-- Enforce behavioral analytics tracking levels at the RPC boundary.
-- This keeps under-13 users fail-closed and restricts FERPA-scoped users/orgs
-- to app_open/route_view even for direct RPC callers.
...
BEGIN
  ...
  FOR kv IN SELECT * FROM jsonb_each(p_props) LOOP
    ...
    IF v_key IN ('message_type', 'file_type', 'file_size_bucket')
