-- Add knowledge_documents to global search (search_org_content RPC).
--
-- knowledge_documents shipped as the 8th RAG source (20261224000000) but the
-- keyword search path never queried it. This recreates search_org_content with a
-- knowledge_rows CTE modeled on album_rows.
--
-- SECURITY — keyword path is broad-only (D1). The vector path (search_ai_documents)
-- is already role-gated via audienceFilterForRole(); this keyword branch is the only
-- new exposure, so it returns ONLY broad docs:
--   COALESCE(audience, 'all') IN ('all', 'both')
-- 'admins'-restricted docs are keyword-invisible to everyone (including admins, who
-- still reach them via the role-gated vector path). Safe-by-default.

-- Trigram index on knowledge title + body.
CREATE INDEX IF NOT EXISTS idx_knowledge_documents_org_search_trgm
...
CREATE INDEX IF NOT EXISTS idx_knowledge_documents_org_created
  ON public.knowledge_documents (organization_id, created_at DESC)
  WHERE deleted_at IS NULL;

-- Recreate search_org_content with knowledge_rows CTE added.
...
BEGIN
  ...
  knowledge_rows AS (
    SELECT
      'knowledge'::text,
      kd.id,
      COALESCE(kd.title, 'Document'),
      left(regexp_replace(COALESCE(kd.body, ''), '\s+', ' ', 'g'), 140),
      '/' || p_org_slug || '/assistant',
      CASE
        WHEN v_short THEN
          CASE WHEN lower(COALESCE(kd.title, '')) LIKE lower(v_prefix) THEN 1.0::real ELSE 0.5::real END
        ELSE GREATEST(
          public.word_similarity(v_trim, COALESCE(kd.title, '') || ' ' || COALESCE(kd.body, ''))::real,
          0.0::real
        )
      END,
      jsonb_build_object('knowledge_id', kd.id),
      kd.created_at
    FROM public.knowledge_documents kd
    WHERE kd.organization_id = p_org_id
      AND kd.deleted_at IS NULL
      -- Broad-only (D1): admins-restricted docs reach admins via the role-gated
      -- vector path only; the keyword path never exposes them.
      AND COALESCE(kd.audience, 'all') IN ('all', 'both')
      AND (
        lower(COALESCE(kd.title, '')) LIKE lower(v_sub)
        OR lower(COALESCE(kd.body, '')) LIKE lower(v_sub)
      )
...
-- Re-issue hardening (SECURITY DEFINER fn must not be PUBLIC-executable).
