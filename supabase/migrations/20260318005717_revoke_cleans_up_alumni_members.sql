-- Fix: When user_organization_roles.status is set to 'revoked', soft-delete
-- the corresponding alumni and members records. Previously the early return
...
BEGIN
  ...
  ELSE
    INSERT INTO public.members (
      ...
      email, photo_url, role, status
    )
    VALUES (
      NEW.organization_id, NEW.user_id, v_first_name, v_last_name,
      v_user_email, v_avatar_url, NEW.role,
  ...
  IF NEW.role = 'alumni' THEN
    ...
    ELSE
       ...
       INSERT INTO public.alumni (
         organization_id, user_id, first_name, last_name, email, photo_url
       )
       VALUES (
         NEW.organization_id, NEW.user_id, v_first_name, v_last_name,
         v_user_email, v_avatar_url
       );
    END IF;
  END IF;
