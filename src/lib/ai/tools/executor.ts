...
const ARG_SCHEMAS: Record<ToolName, z.ZodSchema> = {
  ...
  suggest_mentors: suggestMentorsSchema,
  list_available_mentors: listAvailableMentorsSchema,
...
async function runSuggestMentors(
  sb: SB,
  orgId: string,
  args: z.infer<typeof suggestMentorsSchema>,
  ctx: ToolExecutionContext,
  logContext: AiLogContext
): Promise<ToolExecutionResult> {
  ...
  try {
    const data = await suggestMentors(sb as unknown as import("@supabase/supabase-js").SupabaseClient<import("@/types/database").Database>, orgId, {
...
async function runListAvailableMentors(
  sb: SB,
  orgId: string,
  args: z.infer<typeof listAvailableMentorsSchema>,
  logContext: AiLogContext
): Promise<ToolExecutionResult> {
    ...
    const eligibleMentors = (Array.isArray(mentorProfiles) ? mentorProfiles : [])
      .map((profile) => ({
        ...
        current_mentee_count:
          typeof profile.current_mentee_count === "number"
            ? profile.current_mentee_count
            : 0,
      }))
      .filter(
        (profile): profile is {
    ...
    return await withStageTimeout(`tool_${toolName}`, timeoutMs, async () => {
      switch (toolName) {
        ...
        case "suggest_mentors":
          return runSuggestMentors(
            sb,
            ctx.orgId,
            args as z.infer<typeof suggestMentorsSchema>,
            ctx,
            logContext
          );
        case "list_available_mentors":
          return runListAvailableMentors(
            sb,
            ctx.orgId,
            args as z.infer<typeof listAvailableMentorsSchema>,
            logContext
          );
